...
if (file.exists("data/single_cont_moderator_tests.qs")) {
  single_cont_moderator_tests <- qs::qread("data/single_cont_moderator_tests.qs")
  ...
  qs::qsave(single_cont_moderator_tests, "data/single_cont_moderator_tests.qs")
}
...
```{r meta-regression}
# Cannot save temp file on GitHub due to size
# so instead can download it from the OSF
# If you want to re-estimate the models, set this to false
download_file <- TRUE
...
if (file.exists("data/imp_meta_reg_mods.qs")) {
  ...
  if (download_file == TRUE) {
    download.OSF.file("49gzu", file_name = "data/imp_meta_reg_mods.qs")
    imputed_mods <- try(qs::qread("data/imp_meta_reg_mods.qs"), silent = TRUE)
    if (inherits(imputed_mods, "try-error")) {
      message("Could not load imputed models - will re-impute.")
    }
  }
  
  if (inherits(imputed_mods, "try-error") | !download_file) {
