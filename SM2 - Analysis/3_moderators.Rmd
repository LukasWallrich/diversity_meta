...
For interpretability and comparability with earlier work ... using observed data (so different datasets for each moderator).
...
coefs <- bind_rows(main_plots %>% map_dfr("coefs", .id = "moderator"),
                   tall_plots %>% map_dfr("coefs", .id = "moderator")) %>% select(moderator, domain, everything()) %>% 
...
coefs %>% arrange(moderator, domain) %>% group_by(moderator, domain) %>%  gt() %>% 
  fmt_number(columns = c(beta, se, df), decimals = 2) %>% fmt(p, fns = \(x) fmt_p(x, equal_sign = FALSE)) %>% 
  cols_label(coef = "", beta = "*r*", p = "*p*", se = "SE", .fn = md) %>%
  gt_apa_style() %>% 
...
- Exclude variables with no variance (only positive) and > 95% missing (psych safety and diversity climate)
- How to deal with M*N*AR variables (virtuality and authority diff)? Would require many post-hoc choices - so in spirit of RR, omitted.
- Criterion only coded in few cases, and many were mixed / not applicable - so cannot be meaningfully imputed. Omitted.
...
From here, the process is probabilistic, as effect sizes will be sampled, and then metaCART run repeatedly. This is very slow, so was run on an AWC EC2 instance - see `helpers/aws_code metacart.R`. The code below is just provided for reference - here, results are loaded from file as downloaded from AWS.
...
leaf_counts <- trees_standard %>% 
  imap_dfr(\(mods, dataset) {
    mods %>% discard(is.null) %>% map_int(\(mod) length(mod$n)) %>% tibble(n = .) %>% 
  summarise(mean = mean(n, na.rm = TRUE), median = median(n, na.rm = TRUE), 
            mode = get_mode(n), max = max(n, na.rm = TRUE), min = min(n, na.rm = TRUE), ns = list(n)) %>% 
  mutate(dataset = dataset) 
  }) %>% mutate(cart_method = "standard")  %>% 
  separate(dataset, into = c("imp_method", "domain", "run"), sep = "_") 

leaf_counts %>% group_by(imp_method, domain) %>% 
  summarise(mode = get_mode(unlist(ns)),
            any_moderation = mean(unlist(ns) > 1),
            potential_interactions = mean(unlist(ns) > 2),
            .groups = "drop")

moderator_combinations <- trees_standard %>% 
  imap_dfr(\(mods, dataset) {
    mods %>% discard(is.null) %>% map_chr(\(mod) mod$moderators %>% sort() %>% paste(collapse = " | ") %>% na_if("")) %>% tibble(moderators = .) %>% 
 count(moderators, sort = TRUE) %>% 
  mutate(dataset = dataset) 
  }) %>% mutate(cart_method = "standard")  %>% 
  separate(dataset, into = c("imp_method", "domain", "run"), sep = "_") %>% 
  group_by(imp_method, domain, moderators, cart_method) %>%
  summarise(n = sum(n), .groups = "drop") %>% 
  arrange(imp_method, domain, cart_method, desc(n))


moderator_combinations_long <- moderator_combinations %>% 
    rename(data_domain = domain) %>% 
    mutate(row_id = row_number()) %>%
    separate_rows(moderators, sep = " \\| ") %>%
    mutate(value = TRUE) 

# Which variables are frequently identified as moderators?
moderator_combinations_long %>% 
  group_by(imp_method, data_domain, cart_method, moderators) %>% 
  summarise(n = sum(n), share = sum(n)/500, .groups = "drop") %>% 
  rename(moderator = moderators) %>% 
  filter(!is.na(moderator)) %>% 
  arrange(imp_method, data_domain, cart_method, desc(share)) %>% 
  filter(share >= 0.1)

# Which combinations of at least 2 moderators occur
moderator_combinations_wide <- moderator_combinations_long %>% 
  filter(!is.na(moderators)) %>% 
  pivot_wider(names_from = moderators, values_from = value, 
  values_fill = list(value = FALSE)) %>% 
  select(-row_id) %>%
  mutate(n_mods = rowSums(across(-c(imp_method, data_domain, cart_method, n)))) %>% 
  filter(n_mods > 1)


# Function to sum 'n' for all rows that include at least the specified pattern
sum_n_for_pattern <- function(data, pattern) {
  pattern_data <- data[ , c("n", pattern)]
  matching_rows <- apply(pattern_data[ , -1], 1, function(x) all(x, na.rm = TRUE))
  sum(data$n[matching_rows])
}

# Generating all possible combinations of the logical columns
logical_columns <- names(moderator_combinations_wide)[which(sapply(moderator_combinations_wide, is.logical))]
combinations <- map(2:length(logical_columns), ~combn(logical_columns, .x, simplify = FALSE)) %>% unlist(recursive = FALSE)

# Applying the function for each combination within each group
pattern_sums_grouped <- moderator_combinations_wide %>%
  group_by(data_domain, cart_method) %>%
  group_modify(~map_dfr(combinations, function(comb) {
    pattern <- paste(comb, collapse=" & ")
    sum_n <- sum_n_for_pattern(.x, comb)
    tibble(pattern = pattern, sum_n = sum_n, share = sum_n/1000)
  })) %>%
  ungroup()

pattern_sums_grouped %>% filter(sum_n != 0) %>% 
  arrange(data_domain, cart_method, desc(share)) %>% 
  filter(share >= 0.05)
