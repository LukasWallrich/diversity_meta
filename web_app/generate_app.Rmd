---
title: "Code to create the metaUI web app"
---
...
```{r}
# Load your data
my_data <- readr::read_rds("../SM2 - Analysis/data/full_dataset.rds")
...
```{r}
 # Ensure that all filters are numeric or factor
 filters = c("Domain" = "domain", "Sub-Domain" = "sub_dom",
              "Year" = "year_merged", "Country" = "country_grp",
              "Publication type" = "pub_status",
              "Article focus" = "art_focus", 
              "Top management team" = "tmt", "Student sample" = "stud_sample", 
              "Industry sector" = "ind_sector",
              "Team function" = "team_function",
              "Diversity concept" = "meas_type", "Performance measure" = "objective_rater",
              "Complexity" = "complexity", "Interdependence" = "interdep",
              "Longevity" = "longevity", 
...
my_data <- my_data %>% mutate(
  ...
  se = sqrt(var_adj),
  p_value = 2 * (1 - pt(abs(r_adj/se), n_teams - 2)),
  country_grp = fct_lump_min(country, 3),
  pub_status = case_when(
    pub_status == "Published" ~ "Article/book chapter",
    .default = pub_status
  ),
  art_focus = case_when(
    art_focus == "focal H" ~ "focal hypotheses",
    art_focus == "auxiliary H" ~ "auxiliary hypotheses",
    art_focus == "descriptive" ~ "descriptive results",
    .default = NA
  ),
  objective_rater = fct_collapse(rater, "Objective" = "Objective", 
                                        "Subjective" = c("Subjective - self", "Subjective - supervisor", "Subjective - external"))) %>% 
  mutate(across(filters %>% set_names(NULL), ~if (!is.numeric(.x)) as.factor(.x) else .x))
...
shiny_data <- prepare_data(my_data,
  article_label = "id",                      
  study_label = "articlestudy", 
  es_field = "r_adj", 
  se = "se", 
  pvalue = "p_value",
  sample_size = "n_teams",
...
```{r}
generate_shiny(shiny_data, dataset_name = "diversity and team performance",
               options = list(selection_list_threshold = 4,
                              shiny_theme = "journal"))
```
...
```{r}
generate_shiny(shiny_data, 
               dataset_name = "diversity and team performance",
               # A label for how the effect size should be described in charts
               eff_size_type_label = "r (adjusted for attenutation)",
               # The folder the app should be saved to 
               # if you keep as is, you should get a popup to select it when running the code
               save_to_folder = "shiny_code",
               # Details to show on the about page - see ?create_about for all options
               citation = "To be added after peer review",
               contact = "To be added after peer review",
               osf_link = "https://anonymous.4open.science/r/diversity_meta-5DC0",
               options = list(selection_list_threshold = 3,
                              shiny_theme = "journal")
               )
```
...
-   Moved Analyse & Reset buttons to the top

ToDos:

-   [ ] Add SESOI selection and add that line into charts (consider specific testing)

-   [ ] Add model from meta-analysis and highlight as such (clubSandwich RVE)

-   [ ] Replace ID by citations
...
```{r}
rsconnect::deployApp(appDir = "shiny_code", appName = "diversity_meta")
```
